...
CamCpp is a dual-camera preview pipeline intended for **vertical FOV extension** (stacking two views along the Y axis) and for reducing visible artifacts so the combined output can be used in **VR / electronic vision-assist glasses**.
...
- `MainActivity`  
  Builds the layout, requests permission, owns lifecycle.

- `BackAction`  
  Manages `SurfaceTexture` for back camera preview and applies a transform matrix.

- `UvcAction`  
  Manages `SurfaceTexture` for UVC preview, prepares root access to `/dev/video*`, applies a transform matrix.

**Android APIs used**

- `TextureView`, `SurfaceTexture`, `Surface`, `Matrix`
- `CameraManager` / `CameraCharacteristics`  
  Used to read `SENSOR_ORIENTATION` and pick a back camera ID for orientation logic on the Kotlin side.
...
- **camera2ndk**:  
  `ACameraManager`, `ACameraDevice`, `ACameraCaptureSession`, `ACaptureRequest`
- **mediandk**:  
  `AImageReader` (YUV frames callback)
- **android/native_window**:  
  `ANativeWindow_lock` / `ANativeWindow_unlockAndPost`
- **OpenCV**:  
  `core`, `imgproc` (color conversion, rotation, blur)

#### UVC pipeline

- **Linux V4L2**:  
  `ioctl`, `VIDIOC_*`, `mmap`, `poll`, `/dev/video*`
- **OpenCV**:  
  `imgproc`, `imgcodecs` (YUYV→RGBA conversion or MJPEG decode)

#### Build / link

`CMakeLists.txt` links:

- `camera2ndk`, `mediandk`, `log`, `android`, `dl`, `jnigraphics`
- OpenCV libs (as configured in your project)
...
- Activity is locked to `screenOrientation="landscape"`
...
That means **UVC start is currently coupled to CAMERA permission**, even though V4L2 access itself does not require it.

---

## 3) UI layout and how frames are placed on screen

### Fixed panel and stacked views
...
The panel contains a vertical `LinearLayout` named `stack`.

`stack` contains two `TextureView`s:

- `backTv`: **2160×800**
- `extTv`: **2160×800**

So the intended combined output is:

- **top camera + bottom camera** for vertical FOV expansion.

### Panel scaling to fit the real device screen

`scalePanelToFitIfNeeded()` scales the panel down (**never up**) if the device screen cannot fit `2160×1600` at the chosen position:
...
### Small Y translations (seam/overlap alignment)

- `backTv.translationY = +14`
- `extTv.translationY = -30`

This is a crude seam alignment approach: shifting each preview toward the seam to reduce the visible boundary.
...
1. `AImageReader_new(1280, 720, AIMAGE_FORMAT_YUV_420_888, 4, ...)`
2. `AImageReader_ImageListener.onImageAvailable` pulls frames via `AImageReader_acquireNextImage`
3. Planes are copied into a contiguous buffer in **NV21** layout (Y + interleaved VU):
   - Plane0 → Y
   - Plane2 + Plane1 → interleaved VU (NV21)
4. Decode thread converts NV21 → RGBA using OpenCV:
   - `cv::cvtColor(yuv, rgbaReuse, cv::COLOR_YUV2RGBA_NV21)`

**Back camera format summary**
...
Native decode rotates the back camera frame:

- `cv::rotate(rgbaReuse, rgbaReuse, cv::ROTATE_90_CLOCKWISE);`

So the back camera frame is rotated **90° clockwise** *before rendering*.
...
- Takes the last `BACK_SEAM_PX` rows (default `12`) and applies:
...
> Note: Helper functions for alpha feathering (`applyTopSeamFeather`) and sharpening (`unsharpRect`) exist in `back_camera.cpp`, but are not used in the active pipeline shown.
...
Window buffers are configured as:
...
- prefers driver name starting with `"uvcvideo"` ( Topics: true UVC)
...
1. Prefer **YUYV** over MJPEG
2. Prefer modes that meet desired FPS
3. Prefer higher pixel count (resolution)
4. Prefer higher max FPS
...
When using **YUYV**, the code switches camera exposure to manual and performs a software AE loop:

1. Compute sampled average luma (fast subsampling):
   - `avgLumaYuyvSample()` reads Y from packed YUYV
2. Every `UVC_AE_ADJUST_INTERVAL_MS` (60 ms):
   - Compare `avgLuma` to target `UVC_AE_TARGET_LUMA = 126` with tolerance ±10
3. Adjust exposure and gain using V4L2 controls (if available):
   - `V4L2_CID_EXPOSURE_ABSOLUTE`
   - `V4L2_CID_GAIN`
4. Exposure is capped based on FPS:
   - cap ≈ 65% of frame duration
   - bounded to `[100us .. 16000us]` and device min/max

**Purpose:** stabilize brightness and reduce flicker under changing lighting while keeping exposure short enough to preserve FPS and reduce motion blur.

When using **MJPEG**, the code keeps camera auto exposure and autogain enabled (no custom AE loop).
...
1. Set entire frame alpha to 255
2. For the top `UVC_SEAM_PX` rows (default `12`):
   - Apply `GaussianBlur` over the seam region
   - Apply a vertical alpha ramp from transparent → opaque  
     (row 0 alpha near 0, last seam row near 255)
...
**Important detail:** right now the UI **stacks two TextureViews**, not compositing them into one surface, so this alpha feather only helps if:
...
**Practical impact:** the back preview is stretched/cropped in a way that is not purely aspect-ratio fit/fill. This can produce a “small central image with black borders” or unnatural scaling depending on buffer geometry and rotation.
...
**Tradeoff:** constants are device- and camera-specific. If the chosen UVC mode changes (e.g., 1280×720 vs 1920×1080) the same constants can produce different framing.
...
1. Builds an output band with height `2 * overlapPx`
2. For each row in that band, linearly blends:
   - back strip row (near bottom of back image)
   - ext strip row (near top of UVC image)  
   using `cv::addWeighted`
3. Applies a final `GaussianBlur` to hide transitions
...
- extracts seam strips from the two `TextureView`s into bitmaps, and/or
...
- The `TextureView` transform matrices apply strong non-uniform scaling and offsets  
  (`sx = scale * 2`, `sy = scale / 2`, plus UVC `kx/ky` constants), which can shrink or crop the visible region unexpectedly.
- The back pipeline rotates in native and the Kotlin side may also assume rotation based on `backRotDeg`. If assumptions mismatch, width/height used for scaling becomes wrong.
- The UVC transform deliberately uses `kx ≈ 0.495` (“use about half the nominal width”), which inherently causes letterboxing unless translation/scale are precisely tuned for the current mode.
